...
%   Replicated from the GSPBox for completeness. Use the GSPBox version
%   instead if you need a supported version.
%
...
%     * 'expwin'  : Exponentially decreasing polynomial approximation
%     * 'exact'   : Ideal lowpass filter (Uk*Uk'*R)
%     Default: 'lp-jch'
...
%     * 'std'  : Usual method using dichotomy all the time
%     Default: 'fast'
...
if ~isfield(param, 'pcoefs')
    ...
    if ~isfield(param, 'lk') && ~strcmp(param.filter, 'exact')
        ...
        switch param.lk_est_method
            ...
            case 'std'
                [~, param.lk, ~, ~] = estimate_lambda_k(G, k, param);
    ...
    switch param.filter
        case 'lp-ch'
            [param.pcoefs, ~] = jackson_cheby_poly_coefficients(0, param.lk/2, [0, G.lmax], param.order);
...
tic;
if strcmp(param.filter, 'exact')
    Uk = compute_eigen(G, k);
    filt_sig = Uk*Uk'*param.R;
else

function [ graph_filter ] = approx_filter_jch(G, lk, m, param)
...
graph_filter = @(x) gsp_cheby_eval(x, jch, arange);

% Nf = length(g);
% gt = cell(Nf, 1);
% 
% for ii = 1:Nf
%     graph_filter{ii} = @(x) gsp_cheby_eval(x, jch, arange);
% end
% 
% if Nf == 1;
%     graph_filter = graph_filter{1};
% end

...
% https://arxiv.org/abs/1611.00938
    ...
    tot = 0;
    for ii = 1:N
       fprintf('%0.2f $\\pm$ %0.2f \t', data.measures(ii).mpe, data.measures(ii).mpestd);
       tot = tot + data.measures(ii).mpe;
    end
    fprintf('| %0.2f', tot/N);
    fprintf('\n');
    ...
    tot = 0;
    for ii = 1:N
       fprintf('%0.2f $\\pm$ %0.2f \t', data.measures(ii).mpnosvd, data.measures(ii).mpnosvdstd);
       tot = tot + data.measures(ii).mpnosvd;
    end
    fprintf('| %0.2f', tot/N);
    fprintf('\n');
